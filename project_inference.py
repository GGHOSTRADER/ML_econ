...
def spike_metrics(y_true, y_pred, quantile=0.95, max_lead=10):
    """
    Same spike definition as LightGBM:
...
if __name__ == "__main__":
    ...
    #   test rows:  [val_end, n)
    # For val/test we extend back by (seq_len - 1) for context
    ...
    # Train:
    #   train_df rows: [0 .. train_end-1]
    #   sequences: len(train_df) - seq_len + 1
    #   targets correspond to rows [seq_len-1 .. train_end-1]
    n_train_seq = len(train_ds)
    time_train = full_df.index[seq_len - 1 : seq_len - 1 + n_train_seq]

    # Val:
    #   val_df rows: [train_end - seq_len + 1 .. val_end-1]
    #   first target row = global index train_end
    #   last target row  = global index val_end-1
    n_val_seq = len(val_ds)
    time_val = full_df.index[train_end : train_end + n_val_seq]

    # Test:
    #   test_df rows: [val_end - seq_len + 1 .. n-1]
    #   first target row = global index val_end
    #   last target row  = global index n-1
    ...
    # 11) Spike metrics on TEST (same definition as LightGBM)
    _ = spike_metrics(test_targets, test_preds, quantile=0.95, max_lead=10)

    # 12) Save predictions with timestamps
    pred_df = pd.DataFrame(index=full_df.index)
    pred_df[target_col] = full_df[target_col] * target_scale  # true-scale target

    pred_df.loc[time_train, "transformer_pred_sigma"] = train_preds
    pred_df.loc[time_val, "transformer_pred_sigma"] = val_preds
    pred_df.loc[time_test, "transformer_pred_sigma"] = test_preds

    pred_df.to_pickle("transformer_rv15_predictions.pkl")
    print("\nSaved Transformer predictions to 'transformer_rv15_predictions.pkl'.")

    # 13) Plots on TEST set (full + tail)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "transformer_test_full.png"))
    plt.close()

    # Tail (last 5000 steps)
    zoom_n = 5000
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "transformer_test_tail.png"))
    plt.close()
